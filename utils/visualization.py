...
def visual_segmentation_sets_with_pt(seg, image_filename, opt, pt):
    ...
    if isinstance(image_filename, torch.Tensor):
        image_filename = image_filename.item() if image_filename.dim() == 0 else str(image_filename)
    elif isinstance(image_filename, bytes):
        image_filename = image_filename.decode('utf-8')
    elif isinstance(image_filename, np.ndarray):
        image_filename = str(image_filename)

    img_path = os.path.join(opt.data_path, image_filename + ".png")
    img_path = img_path.replace("Breast-BUSI/", "Breast-BUSI/img/")
